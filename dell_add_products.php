...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO dell_products (name, description, price, mrp, image, brand) VALUES (?, ?, ?, ?, ?, ?)");
...
<div class="container d-flex justify-content-center align-items-start pt-5 pb-5">
  ...
  <h3 class="text-center mb-4 form-label">Add <?= htmlspecialchars($navbarLabels['dell'] ?? 'Dell'); ?> Product</h3>
    <?php echo $message; ?>
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <input type="file" name="image" class="form-control" accept="image/*" placeholder="Upload product image..." onchange="previewImage(event)"  required>
        <img id="preview" style="width:20%; margin-top:10px; border-radius:10px; display:none;">
      </div>

      <div class="d-grid gap-3 mt-3">
        <button type="submit" class="btn btn-primary">🚀 Add Product</button>
        <a href="dell.php" class="btn btn-secondary">← Back to Dell Page</a>
      </div>
    </form>
  </div>
</div>
