...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO boult_products (name, description, price, mrp, image, brand) VALUES (?, ?, ?, ?, ?, ?)");
  ...
  <h3 class="text-center mb-4 form-label">Add <?= htmlspecialchars($navbarLabels['boult'] ?? 'Boult'); ?> Product</h3>
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="d-grid gap-3 mt-3">
        ...
        <a href="boult.php" class="btn btn-secondary">← Back to Boult Page</a>
      </div>
    </form>
  </div>
</div>
