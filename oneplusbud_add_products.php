...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO oneplusbud_products (name, description, price, mrp, image, brand) VALUES (?, ?, ?, ?, ?, ?)");
...
<head>
  ...
  <title>Add OnePlus Product</title>
...
<div class="container d-flex justify-content-center align-items-start pt-5 pb-5">
  ...
  <h3 class="text-center mb-4 form-label">Add <?= htmlspecialchars($navbarLabels['oneplusbud'] ?? 'Oneplus Buds'); ?> Product</h3>
    <?php echo $message; ?>
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <input type="number" name="price" class="form-control" step="1" placeholder="Enter selling price..." required>
      </div>

      <div class="mb-3">
        <label class="form-label">MRP (₹)</label>
        <input type="number" name="mrp" class="form-control" step="1" placeholder="Enter MRP..." required>
      </div>

      <div class="mb-3">
        <label class="form-label">Product Image</label>
        <input type="file" name="image" class="form-control" accept="image/*" placeholder="Choose product image..." onchange="previewImage(event)" required>
        <img id="preview" style="width:20%; margin-top:10px; border-radius:10px; display:none;">
      </div>

      <div class="d-grid gap-3 mt-3">
        <button type="submit" class="btn btn-primary">🚀 Add Product</button>
        <a href="oneplusbud.php" class="btn btn-secondary">← Back to Oneplus Buds Page</a>
      </div>
    </form>
  </div>
</div>
