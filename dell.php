...
if (isset($_GET['add_to_cart'])) {
    ...
    $product_key = 'dell_' . (int)$_GET['add_to_cart'];

    // ✅ If already in cart, increase qty
    if (isset($_SESSION['cart'][$user_id][$product_key])) {
        $_SESSION['cart'][$user_id][$product_key]['quantity'] += 1;
    } else {
        $raw_id = (int)str_replace('dell_', '', $product_key);

        $stmt = $conn->prepare("SELECT * FROM dell_products WHERE id = ? AND deleted_by_admin = 0");
...
<?php if ($delete_mode): ?>
  <div class="container my-3">
    <form id="bulkDeleteForm" method="POST" action="delete_selected_products.php">
      ...
      <input type="hidden" name="table" value="dell_products">
      <input type="hidden" name="redirect_page" value="dell.php">
...
<div class="row px-1">
    <div class="col-md-10 px-3">
        <div class="row px-1 py-1">
            <?php
            $query = "SELECT * FROM dell_products WHERE brand = 'Dell' AND deleted_by_admin = 0";
        ...
        <a href="javascript:void(0);" 
           ...
           style="color:red; font-weight:bold;">Less</a>
                        ...
                        <h6>
                         ...
                         <?php if (!$delete_mode): ?>
  <a href="dell.php?add_to_cart=<?php echo (int)$row['id']; ?>" 
   class="btn btn-info btn-hover">
   Add to Cart
</a>

<a href="dell.php?add_to_cart=<?php echo (int)$row['id']; ?>&buy_now=1" 
