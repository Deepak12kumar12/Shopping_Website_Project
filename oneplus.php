...
if (isset($_GET['add_to_cart'])) {
    ...
    $product_key = 'oneplus_' . (int)$_GET['add_to_cart'];

    // ✅ If already in cart, increase qty
    if (isset($_SESSION['cart'][$user_id][$product_key])) {
        $_SESSION['cart'][$user_id][$product_key]['quantity'] += 1;
    } else {
        $raw_id = (int)str_replace('oneplus_', '', $product_key);

        $stmt = $conn->prepare("SELECT * FROM oneplus_products WHERE id = ? AND deleted_by_admin = 0");
  ...
  <ul class="dropdown-menu dropdown-glass">
    ...
    <?php endif; ?>
      ...
      <li class="nav-item dropdown mx-3">
        ...
        <ul class="dropdown-menu dropdown-glass">
          <li><a href="<?php echo $delete_mode ? 'apple.php?delete_mode=1' : 'apple.php'; ?>"class="dropdown-item nav-glow" ><?= htmlspecialchars($navbarLabels['apple'] ?? 'Apple'); ?></a></li>
          <li><a href="<?php echo $delete_mode ? 'samsung.php?delete_mode=1' : 'samsung.php'; ?>" class="dropdown-item nav-glow" ><?= htmlspecialchars($navbarLabels['samsung'] ?? 'Samsung'); ?></a></li>
          <li><a href="<?php echo $delete_mode ? 'xiaomi.php?delete_mode=1' : 'xiaomi.php'; ?>" class="dropdown-item nav-glow" ><?= htmlspecialchars($navbarLabels['xiaomi'] ?? 'Xiaomi'); ?></a></li>
          <li><a href="<?php echo $delete_mode ? 'oneplus.php?delete_mode=1' : 'oneplus.php'; ?>" class="dropdown-item nav-glow" ><?= htmlspecialchars($navbarLabels['oneplus'] ?? 'OnePlus'); ?></a></li>
...
<?php if ($delete_mode): ?>
  <div class="container my-3">
    <form id="bulkDeleteForm" method="POST" action="delete_selected_products.php">
      ...
      <input type="hidden" name="table" value="oneplus_products">
      <input type="hidden" name="redirect_page" value="oneplus.php">
...
<div class="row px-1">
    <div class="col-md-10 px-3">
        <div class="row px-4 py-4">
            <?php
            $query = "SELECT * FROM oneplus_products WHERE brand = 'Oneplus' AND deleted_by_admin = 0";
        ...
        <a href="javascript:void(0);" 
           ...
           style="color:red; font-weight:bold;">Less</a>
                        ...
                        <h6>
                         ...
                         <?php if (!$delete_mode): ?>
  <a href="oneplus.php?add_to_cart=<?php echo (int)$row['id']; ?>" 
   class="btn btn-info btn-hover">
   Add to Cart
</a>

<a href="oneplus.php?add_to_cart=<?php echo (int)$row['id']; ?>&buy_now=1" 
